    ...
    fetch('https://enjames-ridemyway.herokuapp.com/api/v1/users/profile', {
        ...
        .then((res) => {
            ...
            if (res.status === 'success') {
                ...
                const userId = window.location.search.substr(1).split('=')[1]; //
                fetch(`https://enjames-ridemyway.herokuapp.com/api/v1/users/${userId}`, {
                    method: 'GET',
                    credentials: 'include'
                })
                    .then(resp => resp.json())
                ...
                profileBody.style.marginBottom = '60vh';
            }
        })
        .catch(() => PageFunctions.showMessage('error', 'An error occurred. Please try reloading the page.'));
}, false);
